...
profs_gg + geom_boxplot(aes(x=native, y=eval)) + labs(title= "Boxplot of Corse Evaluations by Native Speaking Status", x="Native Speaking Status", y= "Evaluation Score")
```

This box plot looks at teacher evaluation scores based on whether or not a professor's native language is English. Non-native speakers have a lower median evaluation score, but native English speakers have a larger range and five outliers for low evaluation scores.  
...
To determine an instructors attractiveness a six person panel rated attractiveness and the average ratings were shifted so that the mean was at zero. The correlation between attractiveness and evaluation score is `r round(cor(profs$beauty, profs$eval), 2)` which means there isn't much of a correlation.
...
There are two peaks, one around 8am and another around 5pm. The second peak around 5pm is higher with the average bike rentals over 450. 
...
The average bike rentals peak at different times of the day depending if it is a weekday or weekend. The weekday peak around 8am and 5pm, but on the weekend bike rentals peak around 1pm.
...
ggplot() + geom_col(data= weather_avg, aes(x= factor(weathersit), y=avg, fill= factor(workingday))) + facet_wrap(~workingday, labeller = labeller(workingday = c('0' = 'Non-Workingday', '1' = 'Workingday'))) + labs(x = 'Weather Situation', y = 'Average Bike Rentals', title = 'Bike Rentals Based on Weather Sistuation and Workingday Status') + scale_fill_discrete(name = 'Working Day', labels = c('0' = 'Non-working Day', '1' = 'Working Day')) + scale_x_discrete(labels=c('1' = 'Clear', '2' = 'Cloudy', '3' = 'Light Persipitation'))  + theme(axis.text.x = element_text(angle = 30, hjust = 1))
```

When there is precipitation the average number of bike rentals drops on working and non-working days. On working days there isn't a difference between clear and cloudy days, but on non-working days less people rent bikes on cloudy days compared the clear days. 
...
Ridership on the Capital Metro around UT is much lower on Saturdays and Sundays. On Mondays in September there is lower ridership, but Wednesday, Thursday, and Friday in September have teh highest number of ridership during peak hours.   
...
The morning hours have much lower boarding on weekdays and weekends. There isn't much of a correlation between the temperature and the number of boardings.
\newline\newline\newline\newline
```{r, echo=FALSE, message=FALSE}
...
Musical diversity is calculated by counting the number of unique songs that showed up on the Billboard Top 100. There is a big drop in musical diversity reaching its lowest point in 2001 and growing to one of the highest points in 2020.
...
A ten week hit is a song that was on the Billboard Top 100 for ten consecutive weeks. There are 19 artists with at least 30 different songs that were ten-week hits and Elton John leads with 52 songs. 
